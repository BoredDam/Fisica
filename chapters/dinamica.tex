...
K=\frac{1}{2}mv^2.
\]
Se il corpo aumenta la propria velocità, allora $K$ aumenta; se invece rallenta, $K$ diminuisce.
...
In prossimità della superficie terrestre, la forza peso è una forza conservativa. Il lavoro della forza peso nello spostamento verticale da una quota $y_A$ a una quota $y_B$ vale:
\[
...
L_{\text{peso}} = U_A - U_B.
\]
Ne segue che l'energia potenziale associata alla forza peso può essere scritta come:
\[
U(y) = mgy + C,
\]
dove $C$ è una costante arbitraria. Ponendo come riferimento $U(0)=0$, si ottiene:
\[
U(y) = mgy.
\]
...
K_i + U_i = K_f + U_f.
\]
Poiché nel punto più alto la velocità è nulla:
\[
\frac{1}{2}mv_0^2 = mgh,
\]
da cui:
