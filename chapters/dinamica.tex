...
Poiché l’equazione del moto è lineare e descrive un moto oscillatorio, si cerca una soluzione armonica del tipo
\[
\theta(t) = A \cos(\omega t + \varphi),
\]
dove \(A\) è l’ampiezza dell’oscillazione, \(\omega\) la pulsazione e \(\varphi\) una costante di fase.
...
Sostituendo \(\theta(t)\) e la sua derivata seconda nell’equazione del moto,
\[
...
\[
-\omega^2 A \cos(\omega t + \varphi)
+ \frac{g}{\ell} A \cos(\omega t + \varphi) = 0.
\]

Affinché questa equazione sia soddisfatta per ogni istante di tempo \(t\), e per soluzioni non banali (\(A \neq 0\)), deve annullarsi il coefficiente del termine oscillante. Si ottiene quindi la condizione
\begin{equation}
\boxed{
...
La pulsazione \(\omega\) determina il periodo del moto oscillatorio, che risulta
\[
T = \frac{2\pi}{\omega}
