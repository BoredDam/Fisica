...
\begin{equation}
\boxed{
\theta(X) \equiv \theta_0 \frac{X}{X_0}
}
\end{equation}
\addcontentsline{equ}{myequations}{Temperatura empirica in funzione della variabile termometrica}
...
\end{equation}
\addcontentsline{equ}{myequations}{Temperatura empirica in funzione della pressione}
...
Il calore viene indicato con $Q$ e, se è positivo, indica che il sistema assorbe calore, se è negativo, indica che il sistema cede calore. Si può inoltre introdurre il \emph{calore elementare} $\delta Q$ che rappresenta il calore scambiato in un intervallo di tempo infinitesimo.
...
\end{equation}
\addcontentsline{equ}{myequations}{Legge della trasformazione isoterma}
...
\end{equation}
\addcontentsline{equ}{myequations}{Legge della trasformazione isobara}
...
\end{equation}
\addcontentsline{equ}{myequations}{Legge della trasformazione isocora}
...
\end{equation}
\addcontentsline{equ}{myequations}{Equazione di stato per n moli di gas ideale}
...
\end{equation}
\addcontentsline{equ}{myequations}{Temperatura in funzione dell'energia cinetica media}
...
Si possono introdurre due nuove grandezze, la \textbf{capacità termica specifica} $c$ e la \textbf{capacità termica molare} $C$, che rappresentano rispettivamente il calore necessario per aumentare di un grado Celsius la temperatura di un grammo di sostanza o di una mole di sostanza. Queste grandezze sono legate al calore scambiato da un sistema tramite le relazioni
...
\end{equation}
\addcontentsline{equ}{myequations}{Temperatura di equilibrio tra due solidi}
...
\subsection{Approfondimento: legame tra energia cinetica e temperatura}\label{sec:legame_energiacinetica_temperatura}
...
Poiché ogni urto fornisce un impulso $2mv_x$, l’impulso totale $I_{\text{tot}}$ trasferito alla parete dal gruppo $\vec{v}$ in tempo $dt$ è:

\[
I_{\text{tot}} = \underbrace{\left(\frac{1}{2} n(\vec{v}) v_x dt \right)}_{\text{numero di urti}} \Delta S\,\overbrace{(2mv_x)}^{\text{impulso per urto}}
...
\[
\left<F\right> = \frac{I_{\text{tot}}}{dt} = m n(\vec{v}) v_x^2 \,\Delta S.
\]

La pressione associata a quel gruppo è quindi:

\[
p(\vec{v}) = \frac{\left< F \right>}{\Delta S} = m n(\vec{v}) v_x^2.
\]
...
In una trasformazione \textbf{quasi-statica}, il sistema è sempre in equilibrio meccanico. Se il volume del sistema varia di una quantità infinitesima $dV$, il lavoro elementare di espansione è definito come
...
\end{equation}
\addcontentsline{equ}{myequations}{Lavoro elementare di espansione}

ed è positivo se il sistema compie lavoro sull’ambiente (espansione, $dV>0$) e negativo se il lavoro è compiuto dall’ambiente sul sistema (compressione, $dV<0$).

\paragraph{Derivazione della formula $\delta L = P\,dV$.}

Consideriamo un gas contenuto in un cilindro dotato di un pistone mobile di area $S$. Se il pistone si sposta di una quantità infinitesima $dh$, il gas esercita su di esso una forza
...
Il lavoro elementare compiuto dal gas durante questo spostamento è

\[
\delta L = F\,dh = P\,S\,dh.
\]

Lo spostamento del pistone comporta una variazione infinitesima di volume pari a
...
\paragraph{Forma generale.}

In forma più generale, il lavoro elementare può essere scritto come

\[
\delta L = \int_S p_{\text{ext}}\, dS \, dh,
\]

dove $p_{\text{ext}}$ è la pressione esterna agente su un elemento di superficie $dS$ che si sposta di $dh$ lungo la direzione normale.

Nel caso di una trasformazione quasi-statica:
\begin{itemize}
	\item la pressione esterna è uniforme sulla superficie mobile,
	\item solo la superficie del pistone contribuisce al lavoro,
	\item lo spostamento $dh$ è lo stesso per ogni punto della superficie.
\end{itemize}

Pertanto:

\[
\delta L = \int_S P\, dS\, dh
= P\,dh \int_S dS
= P\,S\,dh
= P\,dV.
\]
...
\end{equation}
\addcontentsline{equ}{myequations}{Lavoro finito tra due stati}
...
Nei sistemi termodinamici, l’energia non è associata soltanto al moto macroscopico o alla posizione del sistema nel suo insieme, ma anche ai moti e alle interazioni \textbf{microscopiche} delle particelle che lo costituiscono.

Si introduce quindi una grandezza di stato detta \textbf{energia interna}, indicata con $U$, che rappresenta l’energia totale associata allo stato microscopico del sistema. Essa include, ad esempio:
\begin{itemize}
	\item l’energia cinetica di agitazione termica delle particelle;
	\item l’energia potenziale dovuta alle interazioni microscopiche tra le particelle.
\end{itemize}

L’energia interna è una \textbf{funzione di stato}: il suo valore dipende unicamente dallo stato termodinamico del sistema (cioè dai parametri di stato come $P, V, T, \dots$) e non dal percorso seguito per raggiungerlo.

\subsubsection*{Motivazione sperimentale}

Consideriamo due trasformazioni diverse $a$ e $b$ che portano un sistema dallo stesso stato iniziale $A$ allo stesso stato finale $B$.
...
Percorrendo la trasformazione $a$ da $A$ a $B$ e tornando indietro lungo la trasformazione inversa di $b$, si ottiene una trasformazione ciclica. Per qualunque trasformazione ciclica, il bilancio energetico complessivo deve annullarsi.

Indicando con $Q$ il calore scambiato e con $L$ il lavoro compiuto dal sistema, per il ciclo vale:
\[
(Q - L)_a - (Q - L)_b = 0,
\]
da cui segue:
...
Questo risultato mostra che la quantità $Q - L$ \textbf{dipende solo dagli stati iniziale e finale}, e non dalla trasformazione seguita. Ciò giustifica l’introduzione di una funzione di stato $U$, tale che:
\begin{equation}
\boxed{
U(B) - U(A) = Q - L.
}
\end{equation}
\addcontentsline{equ}{myequations}{Definizione di energia interna}

\subsection{Enunciato del primo principio della Termodinamica}

Il \textbf{primo principio della termodinamica} è l’estensione del \textbf{principio di conservazione dell’energia} ai sistemi termodinamici, cioè a sistemi che possono scambiare energia con l’ambiente sotto forma di lavoro e di calore.

\begin{quote}
La variazione di energia interna di un sistema è uguale al calore scambiato con l’esterno meno il lavoro compiuto dal sistema sull’esterno:
\begin{equation}
\boxed{
\Delta U = Q - L.
}
\end{equation}
\addcontentsline{equ}{myequations}{Primo principio della Termodinamica}
\end{quote}

In forma differenziale, il primo principio si scrive:
\[
dU = \delta Q - \delta L.
\]

Questa relazione è valida per qualunque trasformazione, sia reversibile sia irreversibile, e indipendentemente dal fatto che essa sia quasi-statica.

\subsection{Esperienza di Joule (1849)}

L’esperienza di Joule fornisce una dimostrazione sperimentale fondamentale del primo principio, mostrando che \textbf{lavoro meccanico} e \textbf{calore} sono due modi diversi di trasferire la stessa grandezza fisica: l’energia.

\subsubsection*{Apparato sperimentale}

Joule utilizza un \textbf{calorimetro} (un recipiente termicamente isolato, riempito d’acqua) all’interno del quale è immerso un sistema di \textbf{palette}. Le palette sono messe in rotazione da un meccanismo collegato a un \textbf{peso} che cade:

\begin{itemize}
	\item il peso perde energia potenziale gravitazionale;
	\item tale energia viene trasformata in lavoro meccanico che fa ruotare le palette;
	\item la rotazione genera attrito viscoso nel fluido, causando un aumento della temperatura dell’acqua.
\end{itemize}

Poiché il calorimetro è idealmente isolato, l’aumento di temperatura dell’acqua è dovuto esclusivamente al lavoro meccanico compiuto sul sistema.
...
\subsubsection*{Equivalenza tra lavoro e calore}

Misurando:
\begin{itemize}
	\item il lavoro totale compiuto dal peso;
	\item il calore assorbito dall’acqua tramite l’aumento di temperatura;
\end{itemize}
Joule osserva una proporzionalità costante tra lavoro e calore.

Storicamente, questo risultato è espresso tramite l’\textbf{equivalente meccanico della caloria}:
\[
\frac{L}{Q} = J \simeq 4.186 \ \text{J/cal}.
\]

Questo mostra che lavoro e calore non sono forme diverse di energia, ma \textbf{diversi modi di trasferire energia}.

\subsubsection*{Trasformazioni cicliche}

L’esperimento può essere interpretato come una trasformazione ciclica, nella quale lo stato finale coincide con quello iniziale. In una trasformazione ciclica vale:
\[
\Delta U = 0.
\]
...
Q - L = 0,
\]
ossia il lavoro totale compiuto dal sistema e il calore totale scambiato si compensano esattamente.

\subsection{Significato fisico}

Il primo principio chiarisce che:
\begin{itemize}
	\item l’energia interna è una funzione di stato;
	\item il calore e il lavoro non sono funzioni di stato, ma dipendono dal percorso;
	\item ogni variazione di energia interna può essere attribuita a uno scambio di calore, a un lavoro, o a entrambi.
\end{itemize}

Esiste un chiaro parallelismo con la meccanica: così come l’energia potenziale dipende solo dalla posizione, l’energia interna dipende solo dallo stato termodinamico del sistema, mentre lavoro e calore descrivono il \emph{modo} in cui l’energia viene trasferita durante una trasformazione.
...
\end{equation}
\addcontentsline{equ}{myequations}{Primo principio in trasformazione adiabatica}
...
\end{equation}
\addcontentsline{equ}{myequations}{Primo principio in trasformazione senza lavoro}
...
\end{equation}
\addcontentsline{equ}{myequations}{Relazione tra pressione e temperatura in trasformazione isocora}
...
\end{equation}
\addcontentsline{equ}{myequations}{Relazione tra pressione e volume in trasformazione isoterma}
...
\begin{itemize}
	...
	\item $V_{fin} < V_{in} \Rightarrow L < 0$ perché l’ambiente compie lavoro sul gas.
\end{itemize}
...
\end{equation}
\addcontentsline{equ}{myequations}{Calore scambiato in trasformazione isoterma}
...
\begin{equation}
\boxed{
\Delta U = U(T_B) - U(T_A) = m \int_{T_A}^{T_B} c(T)dT
}
\end{equation}
\addcontentsline{equ}{myequations}{Variazione di energia interna di un corpo solido}
...
\end{equation}
\addcontentsline{equ}{myequations}{Energia interna in espansione libera}
...
\end{equation}
\addcontentsline{equ}{myequations}{Calore molare a volume costante}
...
\boxed{%
\begin{aligned}
nC_P\,dT &= nR\,dT + nC_V\,dT && \text{(dividendo per $n\,dT$)}\\
C_P      &= C_V + R
\end{aligned}%
}
\end{equation}
\addcontentsline{equ}{myequations}{ Relazione tra calori molari a pressione e volume costante}
...
\end{equation}
\addcontentsline{equ}{myequations}{Relazione tra temperatura e volume in trasformazione adiabatica reversibile}
...
\end{equation}
\addcontentsline{equ}{myequations}{Relazione tra pressione e volume in trasformazione adiabatica reversibile}
...
\end{equation}
\addcontentsline{equ}{myequations}{Calore totale scambiato in ciclo}
...
\end{equation}
\addcontentsline{equ}{myequations}{Definizione di rendimento di una macchina termica}
...
\end{equation}
\addcontentsline{equ}{myequations}{Rendimento del ciclo di Carnot}
...
\end{equation}
\addcontentsline{equ}{myequations}{Teorema di Carnot}
...
\end{equation}
\addcontentsline{equ}{myequations}{Disuguaglianza di Clausius}
...
\end{equation}
\addcontentsline{equ}{myequations}{Disuguaglianza di Clausius in forma integrale}
...
\end{equation}
\addcontentsline{equ}{myequations}{Definizione di entropia}
...
Consideriamo due stati di equilibrio $A$ e $B$ di un sistema termodinamico.
Supponiamo che essi siano collegati da due trasformazioni diverse:

\begin{itemize}
    \item una trasformazione \textbf{irreversibile} $I$ che porta il sistema da $A$ a $B$;
    \item una trasformazione \textbf{reversibile} $II$ che riporta il sistema da $B$ ad $A$.
\end{itemize}

In questo modo si costruisce un \textbf{ciclo chiuso} $A \to B \to A$,
costituito da un tratto irreversibile seguito da uno reversibile.
Questo artificio è necessario perché la disuguaglianza di Clausius
si applica a trasformazioni cicliche.

\subsubsection*{Disuguaglianza di Clausius}

Per qualunque ciclo termodinamico vale la disuguaglianza di Clausius:
\[
\oint \frac{\delta Q}{T} \le 0.
\]

Nel nostro caso, l’integrale ciclico può essere scomposto nella somma
degli integrali lungo i due tratti del ciclo:
\[
\oint \frac{\delta Q}{T}
=
\int_I \frac{\delta Q_{\text{irr}}}{T}
+
\int_{II} \frac{\delta Q_{\text{rev}}}{T}.
\]

Il tratto $II$ è una trasformazione reversibile che va da $B$ ad $A$;
invertendo gli estremi di integrazione si ottiene:
\[
\int_{II} \frac{\delta Q_{\text{rev}}}{T}
=
- \int_A^B \frac{\delta Q_{\text{rev}}}{T}.
\]

Per definizione di entropia, lungo una trasformazione reversibile vale:
\[
\int_A^B \frac{\delta Q_{\text{rev}}}{T}
=
S(B) - S(A).
...
Sostituendo questa espressione nella disuguaglianza di Clausius si ottiene:
\[
\int_I \frac{\delta Q_{\text{irr}}}{T}
-
\bigl[ S(B) - S(A) \bigr]
...
Riordinando i termini segue la relazione fondamentale:
\[
\int_A^B \frac{\delta Q_{\text{irr}}}{T}
\le
S(B) - S(A)
\equiv \Delta S.
\]

Questa disuguaglianza afferma che la variazione di entropia tra due stati
dipende solo dagli stati iniziale e finale, mentre l’integrale
$\int \delta Q / T$ calcolato lungo un percorso irreversibile
è sempre minore o uguale a $\Delta S$.
L’uguaglianza vale solo nel caso di trasformazioni reversibili.

\subsubsection*{Caso particolare: trasformazione adiabatica}

Nel caso di una trasformazione adiabatica si ha:
\[
\delta Q_{\text{irr}} = 0,
\]
...
\Delta S_{\text{adiabatica}} \ge 0.
\]

In particolare:
\begin{itemize}
    \item per un’adiabatica \textbf{reversibile} vale $\Delta S = 0$;
    \item per un’adiabatica \textbf{irreversibile} vale $\Delta S > 0$.
\end{itemize}
...
Da questo segue che l’entropia di un solido può essere scritta come
...
\end{equation}
\addcontentsline{equ}{myequations}{Entropia di un corpo solido}
...
\end{equation}
\addcontentsline{equ}{myequations}{Variazione di entropia in un gas perfetto}
...
\begin{enumerate}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.5\textwidth]{images/image 6.png}
	\end{figure}
	\item Entrambe le molecole a destra ($N_S = 0, N_D = 2)$
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.5\textwidth]{images/image 7.png}
	\end{figure}
\end{enumerate}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\textwidth]{images/image 8.png}
\end{figure}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\textwidth]{images/image 9.png}
\end{figure}
...
\end{equation}
\addcontentsline{equ}{myequations}{Equazione di Gibbs}
...
\end{equation}
\addcontentsline{equ}{myequations}{Distribuzione di Boltzmann}
...
\end{equation}
\addcontentsline{equ}{myequations}{Entropia di Shannon}
...
\subsection{Approfondimento: Entropia dei buchi neri}\label{sec:entropia_buchi_neri}
...
\end{equation}
\addcontentsline{equ}{myequations}{Variazione di entropia allo zero assoluto}
...
\begin{itemize}
	...
	\item Capitoli 10, 11, 12 e 13 di \citetitle{gasparini2019fisica} \cite{gasparini2019fisica}.
	\item Approfondimento \ref{sec:entropia_buchi_neri} basato sul materiale fornito dal prof. Marco Ruggieri.
	\item Approfondimento \ref{sec:legame_energiacinetica_temperatura} basato sul materiale fornito dal prof. Marco Ruggieri. 
\end{itemize}